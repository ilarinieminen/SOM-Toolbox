...
if isstruct(varargin{1}),
    ...
    if ~isstruct(topol),
        % topol not a struct !?
        warning('SOMTOOLBOX:warning', 'Field ''topol'' is not a struct.');
        varargout{1}=varargin{1};
        varargoutC=2;
        nargin_ = nargin;
    elseif ~isfield(topol,'msize') || ~isfield(topol,'lattice'),
        % Field missing?!
        warning('SOMTOOLBOX:warning', 'Invalid topology struct.');
        varargout{1}=topol;
        varargoutC=2;
        nargin_ = nargin;
    else
...
elseif iscell(varargin{1}),
    ...
    % ITN TODO: why is there a shape parameter here anyway?
    c = varargin{1};
